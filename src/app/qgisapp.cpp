...
#include "ecorestoration/qgsecorestorationcontroller.h"
...
static void setTitleBarText_( QWidget &qgisApp )
{
  ...
  caption += QgisApp::tr( "Eco Restoration" );
  ...
  , mSplash( splash )
{
  ...
  QString caption = tr( "Eco Restoration - %1 ('%2')" ).arg( Qgis::version(), Qgis::releaseName() );
  ...
  // Business build: MetaSearch depends on owslib, which is not shipped here.
  // Force-disable it before plugin restoration so startup cannot fail because of it.
  {
    QgsSettings businessPluginSettings;
    businessPluginSettings.setValue( u"/PythonPlugins/MetaSearch"_s, false );
    businessPluginSettings.remove( u"/PythonPlugins/watchDogTimestamp/MetaSearch"_s );
  }
  ...
  // Native environmental protection and ecological restoration workspace.
  // It is created before window state restoration so that its dock and toolbar
  // participate in the normal QGIS layout persistence mechanism.
  mEcoRestorationController = new QgsEcoRestorationController( this );
  //
  ...
  mSplash->showMessage( tr( "Eco Restoration Ready!" ), Qt::AlignHCenter | Qt::AlignBottom, splashTextColor );
  ...
  if ( !mProjOpen )
  {
    // The integrated Jianghe workbench supplies its own welcome/project
    // panel. Keep QGIS' QML welcome screen unloaded here: it imports
    // Qt Quick Controls Material, and on the customized Windows runtime that
    // scene can crash while modal business dialogs are being closed/opened.
    mWelcomeScreen->hide();
  }
  endProfile();
}
...
bool QgisApp::createBlankProjectForIntegratedWorkflow()
{
  ...
  std::unique_ptr<QgsLayerTree> detachedLayerTreeRoot;
  QgsLayerTreeModel *layerTreeModel = mLayerTreeView ? mLayerTreeView->layerTreeModel() : nullptr;
  if ( mLayerTreeView && layerTreeModel )
  {
    mLayerTreeView->setUpdatesEnabled( false );
    mLayerTreeView->setCurrentIndex( QModelIndex() );
    if ( mLayerTreeView->selectionModel() )
      mLayerTreeView->selectionModel()->clear();

    // During QgsProject::clear(), Qt's item views can still process queued
    // update/layout requests which reference indexes backed by soon-to-be
    // deleted layer tree nodes. Temporarily detach the visible model from the
    // project root so no view repaint can dereference old QgsLayerTreeNode
    // pointers while closeProject() removes the current project tree.
    detachedLayerTreeRoot = std::make_unique<QgsLayerTree>();
    layerTreeModel->setRootGroup( detachedLayerTreeRoot.get() );
  }
  closeProject();
  ...
  if ( mLayerTreeView && layerTreeModel )
  {
    if ( QgsLayerTree *projectRoot = qobject_cast<QgsLayerTree *>( prj->layerTreeRoot() ) )
      layerTreeModel->setRootGroup( projectRoot );
    mLayerTreeView->setCurrentIndex( QModelIndex() );
    if ( mLayerTreeView->selectionModel() )
      mLayerTreeView->selectionModel()->clear();
    mLayerTreeView->setUpdatesEnabled( true );
  }
  ...
  // Do not emit QgisApp::newProject here. In the standard desktop workflow
  // that signal is also used to hide the QML welcome screen; the business
  // workbench has already taken over the project UI and performs its own
  // refresh after metadata and phase groups are written.
...
void QgisApp::completeInitialization()
{
  ...
  if ( mEcoRestorationController )
  {
    mEcoRestorationController->activateBusinessStartupLayout();
  }
