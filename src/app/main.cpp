  ...
  switch ( type )
  {
    ...
    case QtWarningMsg:
    {
      ...
      if ( msg.contains( "QXcbClipboard"_L1, Qt::CaseInsensitive )
           ...
           // triggered while opening the simplified style dialog from docked layers panels
           msg.contains( "QWidgetWindow"_L1, Qt::CaseInsensitive )
           || msg.contains( "must be a top level window"_L1, Qt::CaseInsensitive ) )
      {
        break;
      }
...
int main( int argc, char *argv[] )
{
  ...
  // This branded environmental-restoration desktop application is
  // Chinese-first. A command-line --lang value still takes precedence.
  QString translationCode = QStringLiteral( "zh-Hans" );
...
// TODO Fix android
  ...
  QCoreApplication::setOrganizationName( QStringLiteral( "Jibei" ) );
  QCoreApplication::setOrganizationDomain( QStringLiteral( "jibei.local" ) );
  QCoreApplication::setApplicationName( QStringLiteral( "EcoRestoration" ) );
  ...
  const QString applicationDir = QApplication::applicationDirPath();
  const QString localQtPlugins = QDir::cleanPath( applicationDir + QDir::separator() + "qtplugins" );
  const QString localQt6Plugins = QDir::cleanPath( applicationDir + QDir::separator() + "Qt6" + QDir::separator() + "plugins" );
  const QString systemQtPlugins = QDir::cleanPath( QStringLiteral( QT_PLUGINS_DIR ) );

  QCoreApplication::addLibraryPath( localQtPlugins );
  if ( QDir( localQt6Plugins ).exists() )
    QCoreApplication::addLibraryPath( localQt6Plugins );
  if ( QDir( systemQtPlugins ).exists() )
    QCoreApplication::addLibraryPath( systemQtPlugins );

  // QCA scans QCA_PLUGIN_PATH before Qt library paths. Keep the OpenSSL
  // crypto provider discoverable for direct double-click launches too.
  QStringList qcaPluginPaths;
  const QString existingQcaPath = qEnvironmentVariable( "QCA_PLUGIN_PATH" );
  if ( !existingQcaPath.isEmpty() )
    qcaPluginPaths << existingQcaPath.split( QDir::listSeparator(), Qt::SkipEmptyParts );
  qcaPluginPaths << localQtPlugins << localQt6Plugins << systemQtPlugins;
  qcaPluginPaths.removeDuplicates();
  qputenv( "QCA_PLUGIN_PATH", qcaPluginPaths.join( QDir::listSeparator() ).toUtf8() );
#endif
