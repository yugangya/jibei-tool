...
void QgsApplication::installTranslators()
{
  ...
  if ( *sTranslation() != "C"_L1 )
  {
    ...
    // QGIS uses BCP 47 tags (zh-Hans/zh-Hant), while the Qt packages on
    // Windows ship their Simplified/Traditional Chinese catalogs as
    // zh_CN/zh_TW. Resolve that naming difference so standard Qt controls
    // (menus, dialog buttons and context-menu actions) are translated too.
    QString qtTranslationCode = *sTranslation();
    if ( qtTranslationCode == "zh-Hans"_L1 )
      qtTranslationCode = "zh_CN"_L1;
    else if ( qtTranslationCode == "zh-Hant"_L1 )
      qtTranslationCode = "zh_TW"_L1;

    mQtTranslator = std::make_unique<QTranslator>( this );
    if ( mQtTranslator->load( u"qt_"_s + qtTranslationCode, qtTranslationsPath ) )
    {
    ...
    if ( mQtBaseTranslator->load( u"qtbase_"_s + qtTranslationCode, qtTranslationsPath ) )
    {
...
{
  return u":/images/icons/app.ico"_s;
}
